...
void gtfs_create_disk(void*arr[5])
{
	...
	printf("Size of iNODE- %d\n",sizeof(gtfs_inode));    //424
	int n = 4*1024/sizeof(gtfs_inode);                   //9
	printf("Number of iNODEs in one block - %d\n",n);
	printf("Number of iNODEs totally - %d\n\n",n*gtfs_INODE_BLOCKS);   //45
...
int gtfs_stat(gtfs_inode* i)
{
	...
	printf("Name : %s \nInode number: %d \nType : %d \nMode: %d  \nOffset : %d \nUid : %d \nSize : %d \nAccess time: %s \nCreation time : %s \nModification time: %s \nGroup id: %d \nLinks count : %d \nBlocks: %d \n", i->name, i->i_no , i->type, i->i_mode,i->i_offset, i->i_uid, i->i_size, i->i_atime, i->i_ctime, i->i_mtime, i->i_gid, i->i_links_count, i->i_blocks);
